...
export class DatatypeValidator {
     ...
     */
    private userDefinedDatatypeRestrictions: DatatypeRestrictionsMap = new Map();
    ...
    public isValidableType(type: IRI): boolean {
     ...
     * @param type 
     */
    public isNumericType(type: IRI): boolean {
        let numeric: boolean;
        numeric = DatatypeUtils.xsdNumericDatatypes.some(dt => dt.equals(type));
        if (!numeric) {
            let restr: DatatypeRestrictionDescription = this.userDefinedDatatypeRestrictions.get(type.getIRI());
     ...
     * @param type 
     */
    public getDatatypeFacets(type: IRI): ConstrainingFacets {
        let facets: ConstrainingFacets = DatatypeUtils.typeRestrictionsMap.get(type.getIRI());
        if (facets == null) { //datatype not found among the standard restrictions
            facets = DatatypeUtils.notStandardRestrictionsMap.get(type.getIRI());
        }
        if (facets == null) { //datatype not found among the non-standard restrictions
            let restrDesc = this.userDefinedDatatypeRestrictions.get(type.getIRI());
     ...
     * @param type 
     */
    public getDatatypeEnumerations(type: IRI): Literal[] {
        let enumerations: Literal[];
        let restrDesc = this.userDefinedDatatypeRestrictions.get(type.getIRI());
        if (restrDesc != null) {
     ...
     * @param type 
     */
    public isValid(value: Literal, type: IRI): boolean {
        let valid: boolean = true;
        let facets = this.getDatatypeFacets(type);
        if (facets != null) {
            let valueString = value.getLabel();

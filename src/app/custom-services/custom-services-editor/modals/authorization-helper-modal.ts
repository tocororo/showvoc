...
@Component({
    ...
    templateUrl: "./authorization-helper-modal.html",
})
export class AuthorizationHelperModal {
    ...
    ngOnInit() {
        this.languages = SVContext.getSystemSettings().languages;
        ...
        if (this.authorization) { //edit => restore the form with the given authorization
            ...
            * (not captturing in order to ignore ", {lang:...}" and capture just the lang) */
            ...
            if (authMatch == null) {
                ...
                this.basicModals.alert({ key: "STATUS.INVALID_VALUE" }, { key: "CUSTOM_SERVICES.MESSAGES.CANNOT_PARSE_AUTH", params: { auth: this.authorization } }, ModalType.warning);
            } else {
                /* 

...
import { AuthorizationHelperModal } from "./authorization-helper-modal";

@Component({
    selector: "custom-operation-editor-modal",
    templateUrl: "./custom-operation-editor-modal.html",
...
export class CustomOperationEditorModal {
    ...
    constructor(private customServService: CustomServiceServices, private basicModals: BasicModalsServices,
        public activeModal: NgbActiveModal, private modalService: NgbModal, private elementRef: ElementRef) {
    }
    ...
    private authorizationHelper() {
        ...
        const modalRef: NgbModalRef = this.modalService.open(AuthorizationHelperModal, new ModalOptions('lg'));
    ...
    ok() {
        ...
        for (let fieldName in this.form) {
            ...
            if (
                ...
                this.basicModals.alert({ key: "STATUS.INVALID_DATA" }, { key: "CUSTOM_SERVICES.MESSAGES.MISSING_REQUIRED_FIELD", params: { fieldName: fieldName } }, ModalType.warning);
                return;
            } else { //ad hoc checks
                ...
                if (fieldName == "name" && !validNameRegexp.test(field.value)) { //check that the name is valid
                    this.basicModals.alert({ key: "STATUS.INVALID_VALUE" }, { key: "CUSTOM_SERVICES.MESSAGES.INVALID_NAME" }, ModalType.warning);
                    return;
                }
                if (fieldName == "returns") { //check that the type is completed
                    ...
                    if (!TypeUtils.isOperationTypeValid(returnType)) {
                        this.basicModals.alert({ key: "STATUS.INVALID_VALUE" }, { key: "CUSTOM_SERVICES.MESSAGES.INVALID_RETURN_TYPE" }, ModalType.warning);
                        return;
                    }
                }
                    ...
                    if (parameters != null) {
                        for (let param of parameters) {
                            if (param.name == null) { //all parameter names must be provided
                                this.basicModals.alert({ key: "STATUS.INVALID_DATA" }, { key: "CUSTOM_SERVICES.MESSAGES.EMPTY_PARAM_NAME" }, ModalType.warning);
                                return;
                            } else if (!validNameRegexp.test(param.name)) { //all parameter names must be valid variable name
                                this.basicModals.alert({ key: "STATUS.INVALID_VALUE" }, { key: "CUSTOM_SERVICES.MESSAGES.INVALID_PARAM_NAME", params: { param: param.name } }, ModalType.warning);
                                return;
                            } else if (!TypeUtils.isOperationTypeValid(param.type)) {
                                this.basicModals.alert({ key: "STATUS.INVALID_VALUE" }, { key: "CUSTOM_SERVICES.MESSAGES.INVALID_PARAM_TYPE", params: { param: param.name } }, ModalType.warning);
                                return;
                            }
                        }
                    }
                ...
                if (fieldName == "sparql" && !this.queryValid) {
                    this.basicModals.alert({ key: "STATUS.INVALID_VALUE" }, { key: "CUSTOM_SERVICES.MESSAGES.INVALID_SPARQL_QUERY" }, ModalType.warning);
                    return;
                }
            }
        }

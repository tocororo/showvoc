...
export class RemoteRepoEditorModal {
    ...
    allowBatchModify: boolean = false;

    private BATCH_ATTR: string = "batch";
    private MATCH_USERNAME_ATTR: string = "matchUsername";

    constructor(private activeModal: NgbActiveModal, private projectService: ProjectsServices, private basicModals: BasicModalsServices,
    ...
    applyChange(repo: RepositorySummary) {
        if (repo[this.BATCH_ATTR]) {
            ...
            this.projectService.batchModifyRepostoryAccessCredentials(this.project, repo.remoteRepoSummary.serverURL,
                repo[this.MATCH_USERNAME_ATTR], oldUsername, repo.remoteRepoSummary.username, repo.remoteRepoSummary.password)
                .subscribe(
                    () => {
            ...
            this.projectService.modifyRepositoryAccessCredentials(this.project, repo.id, repo.remoteRepoSummary.username, repo.remoteRepoSummary.password)
                .subscribe(
                    () => {

...
export class AlignmentsComponent implements OnInit {
        ...
        this.getDatasetIRI(this.selectedSourceProject).subscribe(
            datasetIRI => {
                ...
                if (this.datasetIRI == null) { //missing IRI for project => initialize it
                    this.basicModals.confirm("DATASETS.STAUTS.MISSING_DATASET_PROFILE", "Unable to find metadata about the dataset '" + this.selectedSourceProject.getName() +
    ...
    refreshProfile() {
        this.basicModals.confirm("DATASETS.ACTIONS.PROFILE_DATASET", "You're going to refresh the metadata about the dataset '" + this.selectedSourceProject.getName() + 

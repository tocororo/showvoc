...
import { GraphClassAxiomFilter, GraphModelRecord } from 'src/app/models/Graphs';
...
export class ModelGraphComponent extends AbstractGraph {
    ...
    */
    private thingNodesMap: { thingNode: ModelNode, linkedRes: AnnotatedValue<Value> }[] = [];
    ...
    ngOnInit() {
        ...
        this.filters.forEach(f => {
            this.filtersDeltaMap[f.property.getValue().getIRI()] = [];
        });
    ...
    protected expandNode(node: Node, selectOnComplete?: boolean) {
        this.graphService.expandGraphModelNode(<IRI>node.res.getValue()).subscribe(
            (graphModel: GraphModelRecord[]) => {
    ...
    protected closeNode(node: Node) {
        ...
        this.filters.forEach(f => {
            let delta = this.filtersDeltaMap[f.property.getValue().getIRI()];
    ...
    private appendLinks(links: Link[]) {
        ...
        links.forEach(l => {
            if (l.res.getValue().equals(RDFS.subClassOf) && l.target.res.getValue().equals(OWL.thing)) {
                ...
                let linkInGraph = this.graph.getLink(l.source.res.getValue(), l.res.getValue(), l.target.res.getValue());
                if (linkInGraph != null) {
                    ...
                    this.filters.forEach(f => {
                        if (f.property.getValue().equals(l.res.getValue())) { //link is about a filtered relation
                            let delta = this.filtersDeltaMap[l.res.getValue().getIRI()]; //here l.res is for sure a filter property since show is false only in that case
    ...
    private removeLinks(links: Link[]) {
        links.forEach(l => {
            //remove the link
            let removingLink = this.graph.getLink(l.source.res.getValue(), l.res.getValue(), l.target.res.getValue());
            ...
            if (!sourceNode.root && sourceNode.incomingNodes.length == 0 && sourceNode.outgoingNodes.length == 0) {
                ...
                if (sourceNode.res.getValue().equals(OWL.thing)) {
            ...
            if (!targetNode.root && targetNode.incomingNodes.length == 0 && targetNode.outgoingNodes.length == 0) {
                ...
                //if removed node is owl:Thing, remove it also from the thingNodesMap
                if (targetNode.res.getValue().equals(OWL.thing)) {
    ...
    public applyFilter(updatedFilter: GraphClassAxiomFilter) {
        let links = this.filtersDeltaMap[updatedFilter.property.getValue().getIRI()];
     ...
     * @param property
     */
    private getLinksDeltaForProperty(links: Link[], property: IRI) {
        let delta: Link[] = [];
        links.forEach(l => {
            if (l.res.getValue().equals(property)) {
                delta.push(l);
            }
        });
        return delta;
     ...
     */
    private retrieveNode(node: ModelNode, relatedRes: AnnotatedValue<Value>): ModelNode {
        let graphNode: ModelNode;
        if (node.res.getAttribute("isDatatype")) { //datatype nodes must not be reused => create a new one each time
            graphNode = node;
            this.graph.addNode(graphNode);
        } else if (node.res.getValue().equals(OWL.thing)) { //owl:Thing node can be reused only if linked with the same resource
            this.thingNodesMap.forEach(tnm => {
                if (tnm.linkedRes.getValue().equals(relatedRes.getValue())) { //a Thing node linked with relatedRes was already created => reuse it
            ...
            graphNode = <ModelNode>this.graph.getNode(node.res.getValue());
    ...
    protected onNodeDblClicked(node: Node) {
        if (this.incrementalExploration) {
            ...
            if (!node.res.getValue().isResource() || !node.res.getAttribute(ResAttribute.EXPLICIT)) {
                return;
            }
            if (node.open) {
                this.closeNode(node);

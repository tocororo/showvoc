...
@Component({
    selector: '[umlNode]',
    templateUrl: "./uml-node.component.html",
    styleUrls: ['../../graph.css']
})
export class UmlNodeComponent extends AbstractGraphNode {
    ...
    private res: AnnotatedValue<Value>;
    ...
    */
    ...
    private initMeasures() {
        ...
        for (let cord of this.node.listPropInfo) {
            if (cord.property.getValue().equals(RDFS.subClassOf)) {
                countsubClassOf++;
            }
        }
    ...
    protected initPropCoord() {
        ...
        for (let prop of this.node.listPropInfo) {
            ...
            if (prop.property.getValue().equals(RDFS.subClassOf)) {
    ...
    private showPropSubClass(prop: PropInfo) {
        return !prop.property.getValue().equals(RDFS.subClassOf);
    }
    ...
    private updateShowImpl(renderingChange: boolean) {
        ...
        for (let i = 0; i < this.node.listPropInfo.length; i++) {
            if (!this.node.listPropInfo[i].property.getValue().equals(RDFS.subClassOf)) {

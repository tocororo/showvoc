...
export class PartitionFilter {
    ...
    selectPartition(partition: PartitionStruct) {
    ...
    areAllUnchecked(rolePartitionsStruct: RolePartitionsStruct) {
     ...
     */
    reset() {
        this.basicModals.confirm({ key: "GRAPHS.PARTITIONS_FILTER.RESET_FILTER" }, "The partition filter will be reset for all the available roles. Are you sure?", ModalType.warning).then(
            confirm => {
                this.rolePartitionsStructs.forEach(rps => {
                    rps.partitions.forEach(p => {
                        p.checked = true;

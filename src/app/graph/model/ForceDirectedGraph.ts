...
export class ForceDirectedGraph {
     ...
     * @param res 
     */
    public getNode(res: Value) {
        if (res instanceof Literal) return null; //literal node should never be reused, return null
        for (let i = 0; i < this.nodes.length; i++) {
            if (this.nodes[i].res.getValue().equals(res)) {
                return this.nodes[i];
            }
        }
        return null;
    ...
    public getLinksFrom(node: Node) {
        ...
        this.links.forEach(l => {
            if (l.source.res.getValue().equals(node.res.getValue())) {
                links.push(l);
            }
        });
        return links;
    ...
    public getLinksTo(node: Node) {
        let links: Link[] = [];
        this.links.forEach(l => {
            if (l.target.res.getValue().equals(node.res.getValue())) {
                links.push(l);
            }
        });
        return links;

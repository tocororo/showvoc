...
export class YasguiComponent {
    ...
    ngAfterViewInit() {
        ...
        //force the update of the content, so init empty query and prevent UI issue with the gutter
        setTimeout(() => {
    ...
    private customPrefixCompleter(yasqe: any, prefixMappings: PrefixMapping[], fetchFromPrefixCC: boolean): any {
        return {
            ...
            get: (token: any, callback: any) => { //callback is the function to which pass the suggested strings if get is async
                ...
                if (fetchFromPrefixCC) {
                    ...
                    $.get(YASQE.Autocompleters.prefixes.fetchFrom, (data) => {
                ...
                searchService.searchResource(token.autocompletionString, ["property"], false, true, false, SearchMode.startsWith).subscribe(
                    (results: AnnotatedValue<IRI>[]) => {
                        ...
                        for (let i = 0; i < results.length; i++) {
                            let uri = results[i].getValue().getIRI();

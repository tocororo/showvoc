...
export class LangPickerComponent implements ControlValueAccessor {
    ...
    constructor(private properties: SVProperties) { }
    ...
    ngOnInit() {
        ...
        if (!this.config.projectAware) {
            this.languageList = SVContext.getSystemSettings().languages.slice();
            ...
            let projectLangs: Language[] = SVContext.getProjectCtx().getProjectSettings().projectLanguagesSetting;
                ...
                if (this.language == undefined) { //no language specified as @Input
                    if (this.config.allowNoLang) {
                        ...
                        if (SVContext.getProjectCtx() != null) {
                            //set the selected based on the priority list
    ...
    writeValue(obj: string) {
        if (this.languageList.length > 0) { //if language list is already init, get the lang from this list
            ...
            this.language = SVContext.getSystemSettings().languages.find(l => l.tag == obj);
        }
